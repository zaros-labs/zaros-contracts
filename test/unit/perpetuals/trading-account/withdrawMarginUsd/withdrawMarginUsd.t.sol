...
contract WithdrawMarginUsd_Unit_Test is Base_Test {
        ...
        external
    {
        ...
        (UD60x18 withdrawnMarginUsdX18, bool isMissingMargin) = perpsEngine.exposed_withdrawMarginUsd(
            tradingAccountId, address(usdToken), marginCollateralPriceUsdX18, amountToWithdrawUsdX18, users.naruto
        );
        ...
        assertEq(
            MockERC20((mockUsdWith10Decimals)).balanceOf(users.naruto),
            expectedBalance.intoUint256(),
            "balanceOf is not correct after withdrawMarginUsd 10"
        );
        ...
        external
    {
        ...
        tradingAccountId = createAccountAndDeposit(amountToDeposit, address(mockUsdWith10Decimals));
        assertEq(MockERC20(address(mockUsdWith10Decimals)).balanceOf(users.naruto), 0, "balanceOf should be zero");
