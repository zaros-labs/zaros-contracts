...
contract RewardsManagerModule is IRewardsManagerModule {
     ...
     */
    function registerRewardsDistributor(address collateralType, address distributor) external override {
        ...
        override
    {
        ...
        if (!rewardIds.contains(rewardId)) {
            revert ParameterError.InvalidParameter("collateralType-distributor", "reward is not registered");
        }

        RewardDistribution.Data storage reward = vault.rewards[rewardId];

        reward.rewardPerShareD18 += reward.distribute(
            vault.currentEpoch().accountsDebtDistribution, amount.toInt(), start, duration
        ).toUint().to128();
        ...
        returns (uint256)
    {
        ...
        bool success =
            vault.rewards[rewardId].distributor.payout(accountId, poolId, collateralType, msg.sender, rewardAmount);
        ...
        emit RewardsClaimed(
            accountId, poolId, collateralType, address(vault.rewards[rewardId].distributor), rewardAmount
        );
        ...
        returns (uint256)
    {
        ...
        return vault.rewards[rewardId].scheduledValueD18.to256().toUint().divDecimal(
            vault.rewards[rewardId].duration.to256().divDecimal(totalShares)
        );
    }

    /**
     * @dev Generate an ID for a rewards distributor connection by hashing its address with the vault's collateral type
     * address and pool id
     */
     ...
     * @inheritdoc IRewardsManagerModule
     */
    function removeRewardsDistributor(uint128 poolId, address collateralType, address distributor) external override {
        ...
        reward.rewardPerShareD18 += reward.distribute(
            pool.vaults[collateralType].currentEpoch().accountsDebtDistribution, 0, 0, 0
        ).toUint().to128();
