...
library DexSwapStrategy {
    ...
    /// @param slippageTolerance The maximum slippage allowed for a swap, expressed in basis points (1% = 100 basis
    /// points).
    // TODO: Review what  needs to be configured at the swap strategy leaf and what is dynamic to build the swap
    // function call data
    struct Data {
        uint128 id;
    ...
    /// @param self The SwapRouter data storage.
    /// @param swapData The calldata to perform the swap.
    /// @return The result of the swap execution.

    function executeSwap(Data storage self, SwapCallData memory swapCallData) internal returns (uint256) {
        // (bool success, bytes memory result) =
        //     self.swapStrategy.call(abi.encode(self.swapFunctonSelector, routerCallData));
